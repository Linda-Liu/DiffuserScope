function u = tvdenoise(f,lambda,iters,lambda2)
...
% Modified by Linda Liu - add 3D volume TV 
% lambda2 - used for anisotropic TV, lambda2=pixel_size_xy/pixel_size_z
...
if length(N) == 2           % 2D image TV denoising
    id = [2:N(1),N(1)];  % down
    iu = [1,1:N(1)-1];   % up
    ir = [2:N(2),N(2)];  % right
    il = [1,1:N(2)-1];   % left
...
elseif length(N) == 3       % 3D volume TV denoising
    id = [2:N(1),N(1)];  % down
    iu = [1,1:N(1)-1];   % up
    ir = [2:N(2),N(2)];  % right
    il = [1,1:N(2)-1];   % left
